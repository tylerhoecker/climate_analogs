...
function check_memory(
    ...
    used_memory = (Base.summarysize(focal_data_cov) + 
                    Base.summarysize(focal_data_mean) +
                     Base.summarysize(analog_pool)) * 1e-9

    nprocesses = Distributed.nprocs()

    environment_memory = used_memory * nprocesses  
    ...
    necessary_memory = (environment_memory + output_memory + sampling_memory) * 1e-9 + 30
    if total_memory <= 30
