...
# function to check if the data fits in memory, and if not, calculate how many tiles we need to split the data into
function check_memory(
  ...
  if haskey(ENV, "SLURM_CPUS_ON_NODE")
    ...
    # otherwise use total memory of the system
  ...
  # anticipate memory needed for sampling, distributing, and output
  ...
  if total_memory <= 30 # this is a magic number that estimates the minimum memory needed to run the script with any data, based on testing with small datasets and monitoring memory usage. If the system does not have at least this much memory
...
# this creates the extents of the tiles based on the number of tiles needed
...
# this function finds the tile ID for a given point based on the tile extents
...
# this function partitions the data into tiles if it does not fit in memory, and returns the tile ID for each point and the extents of each tile
...
# this function filters the analog pool to only include points within max_dist of the focal points in the tile
...
# this function filters the focal data covariance and mean to only include points within the tile extents
...
# this function filters the focal data mean to only include points within the tile extents
